...
def plot_Q1_quadrangulation_mesh(
    DATA_Q1, ti,
    ax=None,
    figsize=(10, 8),
    title=None,
    ...
    """
    Plot the quadrangulation mesh from a DATA_Q1 dictionary at a given timestep.

    Analogous to plot_T1_triangulation_mesh but for 4-node quad elements.
    DATA_Q1 must have the same key structure as DATA_T1 with 4-entry element lists.
    """
    n_ti = len(DATA_Q1['t'])
    if ti < 0 or ti >= n_ti:
        raise ValueError(f"ti={ti} out of range [0, {n_ti - 1}]")

    node_ids = sorted(DATA_Q1['nodes'][0].keys())
    n_nodes  = len(node_ids)
    ...
    def _deformed_xy(time_index):
        ...
        for arr_idx, nid in enumerate(node_ids):
            if magnification != 1.0:
                x0, y0, _ = DATA_Q1['nodes'][0][nid]
                xd, yd, _ = DATA_Q1['nodes'][time_index][nid]
                xs[arr_idx] = (x0 + magnification * (xd - x0)) * node_scale
                ys[arr_idx] = (y0 + magnification * (yd - y0)) * node_scale
            else:
                xd, yd, _ = DATA_Q1['nodes'][time_index][nid]
    ...
    n_elems  = len(elem_ids)
    quads    = np.array([DATA_Q1['elements'][eid] for eid in elem_ids], dtype=int)  # (n_elems, 4)

    norm_areas = np.array(
        [DATA_Q1['elements_area_normalized'][eid][ti] for eid in elem_ids],
        dtype=float,
    )
    ...
    if show_undeformed and ti != 0:
        ...
        for n0, n1, n2, n3 in quads:
            pts = [(xs0[n0], ys0[n0]), (xs0[n1], ys0[n1]),
                   (xs0[n2], ys0[n2]), (xs0[n3], ys0[n3]),
                   (xs0[n0], ys0[n0])]
            segs0.append(pts)
    ...
    patches = []
    for n0, n1, n2, n3 in quads:
        verts = np.array([[xs[n0], ys[n0]], [xs[n1], ys[n1]],
                          [xs[n2], ys[n2]], [xs[n3], ys[n3]]])
        patches.append(Polygon(verts, closed=True))
    ...
    if title is None:
        ...
        sc_str  = f', scale={node_scale:.2g}'   if node_scale  != 1.0 else ''
        cb_str  = ', A/A₀'                       if color_by_area      else ''
        title   = (f'Q1 Quadrangulation — ti={ti}, '
                   f't={DATA_Q1["t"][ti]:.4g}{mag_str}{sc_str}{cb_str}')
    ax.set_title(title)
